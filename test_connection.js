...
async function testConnection() {
    ...
    if (error) {
        ...
        if (error.code === 'PGRST301') {
            console.error('   (Hint: This might be a Row Level Security (RLS) issue if the table is private, but connection itself reached the server.)');
        }
    } else {
        console.log(`✅ Connection Successful! (Latency: ${duration}ms)`);
        console.log(`   Accessed 'customers' table. Row count: ${data === null ? 'Available (Head Request)' : 'Available'}`);
    }
}
