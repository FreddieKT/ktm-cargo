...
const serviceTypes = SERVICE_TYPE_DEFAULTS.map((st) => ({
  ...st,
  icon: SERVICE_TYPE_ICONS[st.value] || Package,
}));
  ...
  const onFormSubmit = async (data) => {
    ...
    const enhancedData = {
      ...
      tracking_number:
        ...
        `BKK${Date.now().toString(36).toUpperCase()}`,
      origin: 'Bangkok',
      destination: 'Yangon',
    };
    ...
    try {
      ...
      const message = err?.message?.includes('capacity')
        ? 'Weight exceeds PO capacity — adjust and retry.'
        : err?.message?.includes('invoice')
          ? 'Invoice number error — apply the DB sequence migration.'
              ...
              {watchedValues.vendor_po_id && (
                <div className="flex items-center gap-2 text-sm animate-in fade-in duration-300">
                  ...
                  <span className="text-blue-600 font-medium">
                    Cost: ฿{watchedValues.vendor_cost_per_kg}/kg
                  </span>
                </div>
              )}
            </div>
