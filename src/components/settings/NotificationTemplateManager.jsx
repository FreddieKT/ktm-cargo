...
import { 
  Mail, Plus, Pencil, Trash2, Eye, FileText, 
  Truck, Package, CheckCircle, CreditCard, Info
} from 'lucide-react';
import { toast } from 'sonner';
import { DEFAULT_TEMPLATES, processTemplate } from '@/components/notifications/ShippingNotificationService';

const TEMPLATE_TYPES = [
  { value: 'shopping_shipping', label: 'Shopping Order - Shipping', icon: Truck, color: 'bg-blue-100 text-blue-800' },
  { value: 'shopping_delivered', label: 'Shopping Order - Delivered', icon: CheckCircle, color: 'bg-emerald-100 text-emerald-800' },
  { value: 'shipment_shipped', label: 'Cargo Shipment - In Transit', icon: Package, color: 'bg-purple-100 text-purple-800' },
  { value: 'shipment_delivered', label: 'Cargo Shipment - Delivered', icon: CheckCircle, color: 'bg-emerald-100 text-emerald-800' },
  { value: 'order_confirmed', label: 'Order Confirmed', icon: FileText, color: 'bg-amber-100 text-amber-800' },
  { value: 'payment_received', label: 'Payment Received', icon: CreditCard, color: 'bg-green-100 text-green-800' }
];
        ...
        <CardContent>
          {templates.length > 0 ? (
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={`p-2 rounded-lg ${typeConfig.color?.split(' ')[0] || 'bg-slate-100'}`}>
                        <TypeIcon className={`w-5 h-5 ${typeConfig.color?.split(' ')[1] || 'text-slate-600'}`} />
                      </div>
                      <div>
                        ...
                        <p className="text-sm text-slate-500">{typeConfig.label || template.template_type}</p>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm text-slate-400 mb-4">Default templates will be used for notifications</p>
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-blue-700 mt-1">
                  If no custom template is configured for a notification type, the system will use built-in default templates.
                  Create custom templates to personalize your customer communications.
                </p>
              </div>
            </div>
          </div>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-4">
            ...
            <div className="flex gap-3 pt-4 border-t">
              ...
              <Button type="button" variant="outline" onClick={handlePreview} disabled={!formData.subject || !formData.body}>
