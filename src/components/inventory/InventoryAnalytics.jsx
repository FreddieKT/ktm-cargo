...
export function analyzeInventoryDemand(item, movements, shipments) {
  ...
  const suggestedReorderPoint = Math.ceil((item.lead_time_days || 7) * adjustedDailyUsage + safetyStock);
  ...
  const daysUntilStockout = adjustedDailyUsage > 0 
    ? Math.floor(item.current_stock / adjustedDailyUsage) 
    : 999;
...
export function getStockStatus(item) {
  if (item.current_stock <= 0) return { status: 'out_of_stock', color: 'bg-rose-100 text-rose-800' };
  if (item.current_stock <= item.reorder_point) return { status: 'low_stock', color: 'bg-amber-100 text-amber-800' };
