        ...
        <div className="max-w-[210mm] mx-auto bg-white text-slate-900 text-sm border-4 border-slate-900 p-8">
            ...
            <div className="flex justify-between items-center border-b-2 border-slate-900 pb-6 mb-6">
                <div>
                    ...
                    <p className="text-xs font-bold uppercase tracking-widest text-slate-500 mt-1">Non-Negotiable</p>
                </div>
            ...
            <div className="grid grid-cols-2 gap-0 border-2 border-slate-900 mb-6">
                <div className="p-4 border-r-2 border-slate-900">
                    <h3 className="text-xs font-bold uppercase bg-slate-100 inline-block px-2 py-1 mb-3">Shipper</h3>
                ...
                <div className="p-4">
                    <h3 className="text-xs font-bold uppercase bg-slate-100 inline-block px-2 py-1 mb-3">Consignee</h3>
                    <p className="font-bold text-lg">{shipment.customer_name}</p>
                    <p className="whitespace-pre-wrap text-slate-700 mt-1">{shipment.delivery_address || 'Yangon, Myanmar'}</p>
            ...
            <table className="w-full border-2 border-slate-900 mb-6">
                ...
                <tbody>
                    <tr>
                        ...
                        <td className="py-4 px-4 text-center border-r border-slate-300 font-bold">{shipment.weight_kg} kg</td>
                        <td className="py-4 px-4 border-r border-slate-300">
                            <p className="font-medium">{shipment.items_description || 'General Cargo'}</p>
                            {shipment.notes && <p className="text-xs text-slate-500 mt-1 italic">{shipment.notes}</p>}
                        </td>
            ...
            <div className="mt-12 pt-6 border-t-2 border-slate-900">
                <div className="grid grid-cols-2 gap-12">
                    <div>
                        ...
                        <p className="text-xs text-slate-500 mt-1">I certify that the particulars on the face hereof are correct and that insofar as any part of the consignment contains dangerous goods, such part is properly described by name and is in proper condition for carriage by air according to the applicable Dangerous Goods Regulations.</p>
                    </div>
                    <div>
                        <p className="text-xs font-bold uppercase mb-12">Carrier's Signature</p>
                        <div className="border-b border-slate-900"></div>
                        <p className="text-xs text-slate-500 mt-1">Executed on {format(new Date(), 'dd MMM yyyy')} at Bangkok, Thailand</p>
                    </div>
                </div>
            </div>
        </div>
