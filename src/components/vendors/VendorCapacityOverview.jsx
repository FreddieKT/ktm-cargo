...
export default function VendorCapacityOverview({ vendors = [], purchaseOrders = [] }) {
  ...
  const totalAllocated = cargoVendors.reduce((sum, v) => sum + (v.current_month_allocated_kg || 0), 0);
    ...
    <div className="space-y-4">
      ...
      <Card className="border-0 shadow-sm bg-gradient-to-br from-slate-50 to-slate-100">
        <CardHeader className="pb-2">
          ...
          <CardDescription>
            Monthly capacity across all cargo carriers
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium">{totalAllocated.toLocaleString()} / {totalCapacity.toLocaleString()} kg</span>
            </div>
            <Progress 
          ...
          <CardContent className="p-4">
            ...
            <div className="space-y-2">
              {lowCapacityVendors.map(vendor => {
                const usage = ((vendor.current_month_allocated_kg || 0) / vendor.monthly_capacity_kg) * 100;
                const remaining = vendor.monthly_capacity_kg - (vendor.current_month_allocated_kg || 0);
                return (
                  <div key={vendor.id} className="flex items-center justify-between p-2 bg-amber-50 rounded-lg">
                    <div>
                      <p className="font-medium">{vendor.name}</p>
                      <p className="text-xs text-slate-500">Only {remaining.toLocaleString()} kg remaining</p>
                    </div>
                    <Badge className={usage >= 100 ? 'bg-rose-100 text-rose-800' : 'bg-amber-100 text-amber-800'}>
          ...
          <div className="space-y-4">
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <p className="text-sm text-slate-500">
                        ฿{vendor.cost_per_kg || 0}/kg standard • {vendor.lead_time_days || 3}d lead time
                      </p>
                    </div>
                    <Badge className={
                      usagePercent >= 100 ? 'bg-rose-100 text-rose-800' :
                      usagePercent >= 80 ? 'bg-amber-100 text-amber-800' :
                  ...
                  {capacity > 0 ? (
                    <>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span>{allocated.toLocaleString()} / {capacity.toLocaleString()} kg</span>
                      </div>
                      ...
                      <div className="flex gap-4 mt-3 text-xs">
                        ...
                        {vendor.cost_per_kg_express > 0 && (
                          <div className="flex items-center gap-1">
                            ...
                            <span className="font-medium text-amber-600">฿{vendor.cost_per_kg_express}/kg</span>
                          </div>
                        )}
                        {vendor.cost_per_kg_bulk > 0 && (
                          <div className="flex items-center gap-1">
                            <span className="text-slate-400">Bulk ({vendor.bulk_threshold_kg}+ kg):</span>
                            <span className="font-medium text-emerald-600">฿{vendor.cost_per_kg_bulk}/kg</span>
                          </div>
                        )}
                      </div>
                    </>
            ...
            {cargoVendors.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-slate-400">Add vendors with cargo carrier type to track capacity</p>
              </div>
            )}
          </div>
        </CardContent>
