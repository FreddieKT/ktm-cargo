...
export const VALUE_TIERS = {
  VIP: { key: 'vip', label: 'VIP', color: 'bg-purple-100 text-purple-800', minSpent: 100000, minShipments: 20 },
  HIGH: { key: 'high', label: 'High Value', color: 'bg-emerald-100 text-emerald-800', minSpent: 50000, minShipments: 10 },
  MEDIUM: { key: 'medium', label: 'Medium Value', color: 'bg-blue-100 text-blue-800', minSpent: 10000, minShipments: 3 },
  LOW: { key: 'low', label: 'Low Value', color: 'bg-slate-100 text-slate-800', minSpent: 0, minShipments: 0 },
};

export const BEHAVIORAL_SEGMENTS = {
  LOYAL: { key: 'loyal', label: 'Loyal', color: 'bg-amber-100 text-amber-800', icon: 'Crown' },
  AT_RISK: { key: 'at_risk', label: 'At Risk', color: 'bg-rose-100 text-rose-800', icon: 'AlertTriangle' },
  ...
  RETURNING: { key: 'returning', label: 'Returning', color: 'bg-green-100 text-green-800', icon: 'RefreshCw' },
};
...
export function analyzeCustomer(customer, shipments) {
  ...
  const lastShipmentDate = sortedShipments.length > 0 
    ? new Date(sortedShipments[0].created_date) 
    : null;
  
  const firstShipmentDate = sortedShipments.length > 0 
  ...
  const orderFrequency = customerAgeDays > 0 
    ? (shipmentCount / customerAgeDays) * 30 
    : 0;
  ...
  } else if (totalSpent >= VALUE_TIERS.HIGH.minSpent || shipmentCount >= VALUE_TIERS.HIGH.minShipments) {
    valueTier = VALUE_TIERS.HIGH;
  } else if (totalSpent >= VALUE_TIERS.MEDIUM.minSpent || shipmentCount >= VALUE_TIERS.MEDIUM.minShipments) {
...
export function getSegmentSummary(analyzedCustomers) {
  ...
  summary.totals.avgScore = analyzedCustomers.length > 0 
