...
import { 
  ...
  Box, Truck, ArrowUpCircle, ArrowDownCircle, RefreshCw, Bell
} from 'lucide-react';
import { Skeleton } from "@/components/ui/skeleton";
import { toast } from "sonner";
import { format } from 'date-fns';
import { analyzeInventoryDemand, getStockStatus, getReorderAlerts } from '@/components/inventory/InventoryAnalytics';
...
export default function Inventory() {
  ...
  const createMovementMutation = useMutation({
    mutationFn: async (data) => {
      ...
      await base44.entities.InventoryItem.update(item.id, {
        ...
        ...(data.movement_type === 'in' ? { last_restock_date: format(new Date(), 'yyyy-MM-dd') } : {})
      });
  ...
  // Calculate analytics
  const alerts = useMemo(() => getReorderAlerts(items, movements, shipments), [items, movements, shipments]);
  ...
  const [form, setForm] = useState({
    name: '', sku: '', category: 'packaging', current_stock: 0,
    unit: 'pieces', reorder_point: 10, reorder_quantity: 50,
    unit_cost: 0, supplier: '', lead_time_days: 7
  });

  const [movementForm, setMovementForm] = useState({
    movement_type: 'in', quantity: 0, reference_type: 'manual', notes: ''
  });
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="h-auto flex-wrap p-1 gap-1">
            ...
            <TabsTrigger value="alerts" className="gap-1 sm:gap-2 text-xs sm:text-sm">
              ...
              Alerts {alerts.length > 0 && <Badge className="bg-rose-500 text-[10px] sm:text-xs px-1.5">{alerts.length}</Badge>}
            </TabsTrigger>
                      ...
                      <CardContent className="p-5">
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-lg ${categoryConfig[item.category]?.color || 'bg-slate-100'}`}>
                        ...
                        <div className="mb-3">
                          <div className="flex justify-between text-sm mb-1">
                            ...
                            <span className="font-medium">{item.current_stock} / {item.reorder_point * 3} {item.unit}</span>
                          </div>
                          <Progress 
                            value={Math.min((item.current_stock / (item.reorder_point * 3)) * 100, 100)} 
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm mb-4">
                          <div>
                            ...
                            <p className="font-medium">{analytics.dailyUsage} {item.unit}</p>
                          </div>
                          <div>
                            <p className="text-slate-500">Days Left</p>
                            <p className={`font-medium ${analytics.daysUntilStockout < 14 ? 'text-rose-600' : ''}`}>
                              {analytics.daysUntilStockout > 100 ? '100+' : analytics.daysUntilStockout}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex gap-2">
                          <Button 
                            ...
                            variant="outline" 
                            className="flex-1"
                            onClick={() => { setSelectedItem(item); setMovementForm({ ...movementForm, movement_type: 'out' }); setShowMovement(true); }}
                          >
                          ...
                          <Button 
                            ...
                            onClick={() => { setSelectedItem(item); setMovementForm({ ...movementForm, movement_type: 'in' }); setShowMovement(true); }}
                          >
              ...
              <div className="space-y-3">
                {alerts.map(({ item, analytics }) => (
                  <Card key={item.id} className={`border-l-4 ${
                    analytics.urgency === 'critical' ? 'border-l-rose-500 bg-rose-50' :
                    analytics.urgency === 'high' ? 'border-l-amber-500 bg-amber-50' :
                      ...
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <Badge className={
                              analytics.urgency === 'critical' ? 'bg-rose-500' :
                              analytics.urgency === 'high' ? 'bg-amber-500' : 'bg-blue-500'
                            }>
                          ...
                          <p className="text-sm text-slate-600">
                            ...
                            {analytics.daysUntilStockout <= 0 ? ' Out of stock!' : ` ${analytics.daysUntilStockout} days until stockout`}
                          </p>
                        </div>
                        <div className="flex items-center gap-4">
                          <div className="text-right">
                            <p className="text-xs text-slate-500">Suggested Reorder</p>
                            <p className="font-bold text-lg">{analytics.suggestedReorderQty} {item.unit}</p>
                          </div>
                          <Button 
                            size="sm"
                            onClick={() => { 
                              setSelectedItem(item); 
                              setMovementForm({ movement_type: 'in', quantity: analytics.suggestedReorderQty, reference_type: 'restock', notes: '' }); 
                    ...
                    <tbody>
                      {movements.slice(0, 20).map(m => (
                        <tr key={m.id} className="border-t">
                          <td className="p-4 text-slate-500">{format(new Date(m.created_date), 'MMM d, HH:mm')}</td>
                          <td className="p-4 font-medium">{m.item_name}</td>
                          <td className="p-4">
                            <Badge className={m.movement_type === 'in' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'}>
        ...
        <Dialog open={showForm} onOpenChange={setShowForm}>
          <DialogContent className="max-w-md">
            ...
            <form onSubmit={(e) => { e.preventDefault(); createItemMutation.mutate(form); }} className="space-y-4 mt-4">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <Button type="submit" className="flex-1 bg-blue-600">Add Item</Button>
              </div>
            </form>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={showMovement} onOpenChange={(v) => { setShowMovement(v); if (!v) setSelectedItem(null); }}>
          <DialogContent className="max-w-sm">
            <DialogHeader>
              <DialogTitle>{movementForm.movement_type === 'in' ? 'Restock' : 'Use Stock'}: {selectedItem?.name}</DialogTitle>
            </DialogHeader>
            <form onSubmit={(e) => { 
              e.preventDefault(); 
              ...
              <div>
                ...
                <Select value={movementForm.reference_type} onValueChange={(v) => setMovementForm({ ...movementForm, reference_type: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="flex gap-3 pt-4">
                <Button type="button" variant="outline" onClick={() => setShowMovement(false)} className="flex-1">Cancel</Button>
                <Button type="submit" className={`flex-1 ${movementForm.movement_type === 'in' ? 'bg-emerald-600' : 'bg-rose-600'}`}>
