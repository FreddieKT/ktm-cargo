...
import { 
  ...
  ArrowRight, Plus, ShoppingBag, Truck,
  AlertCircle, Clock, Crown, Star, Sparkles
} from 'lucide-react';
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Skeleton } from "@/components/ui/skeleton";
import { segmentCustomers, getSegmentSummary, getMarketingRecommendations } from '@/components/customers/CustomerSegmentationEngine';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!alertsChecked.current && customers.length > 0) {
      ...
      base44.auth.me().then(user => {
        if (user?.email && (segmentSummary.byBehavior.at_risk.count >= 3 || segmentSummary.byBehavior.lapsed.count >= 5)) {
  ...
  const pendingShipments = shipments.filter(s => ['pending', 'confirmed', 'picked_up'].includes(s.status));
          ...
          <CardContent className="px-4 sm:px-6">
            <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-2 sm:gap-3">
              <div className="p-2 sm:p-3 bg-purple-50 rounded-xl">
                ...
                <p className="text-lg sm:text-2xl font-bold text-purple-900">{segmentSummary.byValueTier.vip.count}</p>
                <p className="text-[10px] sm:text-xs text-purple-600 hidden sm:block">฿{segmentSummary.byValueTier.vip.revenue.toLocaleString()}</p>
              </div>
              <div className="p-2 sm:p-3 bg-emerald-50 rounded-xl">
                ...
                <p className="text-lg sm:text-2xl font-bold text-emerald-900">{segmentSummary.byValueTier.high.count}</p>
                <p className="text-[10px] sm:text-xs text-emerald-600 hidden sm:block">฿{segmentSummary.byValueTier.high.revenue.toLocaleString()}</p>
              </div>
              <div className="p-2 sm:p-3 bg-amber-50 rounded-xl">
                ...
                <p className="text-lg sm:text-2xl font-bold text-amber-900">{segmentSummary.byBehavior.loyal.count}</p>
              </div>
              <div className="p-2 sm:p-3 bg-sky-50 rounded-xl">
                ...
                <p className="text-lg sm:text-2xl font-bold text-sky-900">{segmentSummary.byBehavior.new.count}</p>
              </div>
              <div className="p-2 sm:p-3 bg-rose-50 rounded-xl">
                ...
                <p className="text-lg sm:text-2xl font-bold text-rose-900">{segmentSummary.byBehavior.at_risk.count}</p>
              </div>
              <div className="p-2 sm:p-3 bg-gray-50 rounded-xl">
                ...
                <p className="text-lg sm:text-2xl font-bold text-gray-900">{segmentSummary.byBehavior.lapsed.count}</p>
              </div>
            </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-amber-900 mt-1">{pendingShipments.length}</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-blue-900 mt-1">{inTransitShipments.length}</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-emerald-900 mt-1">{deliveredShipments.length}</p>
                </div>
