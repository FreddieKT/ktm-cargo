...
function ShoppingOrderFormDialog({ open, onClose, editingOrder }) {
  const form = useForm({
    ...
    defaultValues: shoppingOrderDefault,
  });
...
**Coverage targets — important paths:** 6. `vendor-registration.spec.js` — new vendor self-registers 7. `price-calculator.spec.js` — quote estimation flow 8. `tasks.spec.js` — task creation and completion
...
1. **Lazy-load heavy chart components inside pages** — especially `Reports.jsx` (1802L), `Procurement.jsx` (1085L):
   ```jsx
   const ShipmentForecasting = lazy(() => import('@/components/reports/ShipmentForecasting.jsx'));
   ```
